...
namespace task_final {
	public class Program {
		public static void Main(string[] args) {
			...
			builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme).AddCookie(options => { options.Cookie.Name = "authentication"; options.Cookie.HttpOnly = true; options.LoginPath = new PathString("/Login/Main"); options.LogoutPath = new PathString("/Login/Main"); options.AccessDeniedPath = new PathString("/Home/Main"); options.ExpireTimeSpan = TimeSpan.FromMinutes(30); options.SlidingExpiration = false; });
			builder.Services.AddDistributedMemoryCache();
			builder.Services.AddSession(options => { options.Cookie.Name = "account_id"; options.IdleTimeout = TimeSpan.FromMinutes(30); options.Cookie.HttpOnly = true; options.Cookie.IsEssential = true; });
